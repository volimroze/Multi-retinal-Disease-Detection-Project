...
# Template CSV must contain "id" column and 3 disease columns (e.g., D,G,A or DR,Glaucoma,AMD)
# ========================
...
# IMPORTANT FIX:
# - Must be a CLASS passed to ResNet (not a lambda/function)
# - Must expose .expansion attribute
# ========================
class SEBasicBlock(BasicBlock):
    expansion = BasicBlock.expansion  # make sure ResNet can read it

    def __init__(self, *args, se_reduction: int = 16, **kwargs):
        super().__init__(*args, **kwargs)
        self.se = SEBlock(self.conv2.out_channels, reduction=se_reduction)
...
def build_se_resnet18(num_classes: int = 3, se_reduction: int = 16):
    # Create a concrete block class that "bakes in" se_reduction,
    # so ResNet receives a proper class (with .expansion).
    class SEBasicBlockFixed(SEBasicBlock):
        def __init__(self, *args, **kwargs):
            super().__init__(*args, se_reduction=se_reduction, **kwargs)

    model = ResNet(
        block=SEBasicBlockFixed,
...
# ========================
# Task 3.1: SE attention full fine-tuning
# ========================
def task3_1_se_finetune_and_submit(
    train_csv="train.csv",
    val_csv="val.csv",
    ...
    best_ckpt_name="task3_1_se_best_resnet18.pt",
    ...
    # Build model (SE-ResNet18)
    model = build_se_resnet18(num_classes=3, se_reduction=se_reduction).to(device)

    # Load pretrained ResNet18 weights (SE adds new params -> strict=False)
    state_dict = torch.load(pretrained_ckpt, map_location=device)
    missing, unexpected = model.load_state_dict(state_dict, strict=False)
    print("\nLoaded pretrained ckpt with strict=False (expected for SE):")
    print(f"  Missing keys (new SE params): {len(missing)}")
    ...
    # Train with best-val saving
    os.makedirs(save_dir, exist_ok=True)
    best_path = os.path.join(save_dir, best_ckpt_name)
    best_val_loss = float("inf")

    print("\nTask 3.1: Full fine-tuning with SE attention (train.csv -> val.csv)")
    for epoch in range(epochs):
        ...
        print(f"[se_resnet18] Epoch {epoch+1}/{epochs} | Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f}")
    ...
    # ========================
    print("\nEvaluating best Task 3.1 model on OFFSITE test set (labels available)")
    ...
    compute_and_print_metrics(y_true_np, y_pred_np, backbone_name="se_resnet18_task3_1")

    # ========================
    # Kaggle ONSITE submission
    # ========================
    print("\nGenerating Kaggle ONSITE submission CSV for Task 3.1...")
    ...
    out_csv = "task3_1_onsite_submission.csv"
...
if __name__ == "__main__":
    set_seed(42)
    task3_1_se_finetune_and_submit(
        epochs=10,
        batch_size=16,
        lr=1e-5,
        img_size=256,
