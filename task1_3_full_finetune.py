...
# CSV: first col = image filename, next 3 cols = labels
# ========================
...
# Template must contain column "id"
# ========================
    ...
    def __init__(self, template_csv, image_dir, transform=None):
        ...
        if "id" not in self.df.columns:
            raise ValueError(
                ...
                "Use the original onsite_test_submission.csv (do not edit in Excel)."
            )
        self.image_dir = image_dir
        self.transform = transform
...
# Train on train.csv, select by val.csv
# Evaluate on offsite_test.csv
# Generate Kaggle onsite submission
# ========================
def task1_3_full_finetune_and_submit(
    train_csv="train.csv",
    val_csv="val.csv",
    ...
    finetuned_ckpt_name="task1_3_best_resnet18_full.pt",
    ...
    onsite_out_csv="task1_3_onsite_submission.csv",
):
    ...
    # load pretrained backbone weights (given by professor)
    ...
    best_path = os.path.join(save_dir, finetuned_ckpt_name)
    best_val_loss = float("inf")

    print("\nTask 1.3: FULL fine-tuning (train on train.csv, validate on val.csv)")
    for epoch in range(epochs):
        ...
        if val_loss < best_val_loss:
            ...
            print(f"Saved best full fine-tuned model to: {best_path}")

    # ---- OFFSITE evaluation ----
    print("\nEvaluating best model on OFFSITE test set (labels available)")
    ...
    compute_and_print_metrics(y_true_np, y_pred_np, tag="resnet18_full_finetune")

    # ---- ONSITE submission ----
    print("\nGenerating Kaggle ONSITE submission CSV...")
    ...
    out_df.to_csv(onsite_out_csv, index=False)
    print(f"Saved: {onsite_out_csv}")
    print(out_df.head())


if __name__ == "__main__":
    set_seed(42)
    task1_3_full_finetune_and_submit(
        epochs=10,
        batch_size=16,
        lr=1e-5,
        img_size=256,
